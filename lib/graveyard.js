...
export async function fetchBurnedCommoners() {
  ...
  for (const asset of burntAssets) {
    ...
    // Both conditions required: burnt flag AND a confirmed Graveyard soulbound token.
    // Helius DAS can return stale/incorrect burnt flags — the soulbound token is the
    // authoritative on-chain proof that a burn actually occurred.
    if (!graveyardMint) continue;
