...
export async function fetchBurnedCommoners() {
  ...
  for (const asset of burntAssets) {
    ...
    // Try to find a matching Graveyard soulbound token by name. These only
    // exist for the original burn-ceremony burns; burns that happen via the
    // PrimeVil transfer mechanism don't mint a soulbound counterpart. Missing
    // soulbound token is OK — the DAS `burnt` flag is authoritative.
    const graveyardAsset = graveyardIndex[original.name.toLowerCase()] ?? null;
    const graveyardMint = graveyardAsset?.id ?? null;

    // Burn date from Graveyard token creation timestamp when available,
    // otherwise null (card renders without a date).
