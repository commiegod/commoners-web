...
const SIGMA32 = u32(new TextEncoder().encode("expand 32-byte k"));

const SS = "phantom_dl_"; // sessionStorage key prefix
...
/**
 * Get or create the dApp's X25519 keypair, persisted in sessionStorage.
 * The same keypair is reused for the session to allow decrypting callbacks.
 */
export function getDappKeypair() {
  if (typeof window === "undefined") return null;

  const stored = sessionStorage.getItem(SS + "keypair");
  if (stored) {
    try {
...
/**
 * Save the established Phantom session to sessionStorage.
...
export function saveDeeplinkSession({ publicKey, session, phantomPublicKey, boxKey }) {
  ...
  sessionStorage.setItem(SS + "phantom_pubkey", phantomPublicKey);
  sessionStorage.setItem(SS + "box_key", JSON.stringify(Array.from(boxKey)));
}

/**
 * Load an existing Phantom session from sessionStorage.
...
export function loadDeeplinkSession() {
  ...
  const phantomPublicKey = sessionStorage.getItem(SS + "phantom_pubkey");
...
/** Clear the Phantom deep link session from sessionStorage. */
export function clearDeeplinkSession() {
  ...
  [
    ...
    "phantom_pubkey",
    "box_key",
  ].forEach((k) => sessionStorage.removeItem(SS + k));
}
