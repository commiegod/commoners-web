...
/**
 * Build the Phantom deep link signTransaction URL.
 * Phantom signs the transaction and returns the signed bytes; the caller
 * is responsible for broadcasting via connection.sendRawTransaction().
 * More widely supported than signAndSendTransaction across Phantom versions.
 * @param {object} opts
 * @param {Uint8Array} opts.serializedTx - serialized Transaction (requireAllSignatures: false)
 * @param {string} opts.session - session token from the connect response
 * @param {Uint8Array} opts.boxKey - precomputed box key
 * @param {string} opts.redirectLink - full callback URL (with action=signTransaction)
 */
export function buildSignTransactionURL({ serializedTx, session, boxKey, redirectLink }) {
  const { publicKey } = getDappKeypair();
  const dappPublicKeyBase58 = bs58.encode(Buffer.from(publicKey));
  ...
  return `https://phantom.app/ul/v1/signTransaction?${params}`;
}
