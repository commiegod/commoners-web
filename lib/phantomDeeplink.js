...
/**
 * Build the Phantom deep link signAndSendTransaction URL.
 * Phantom signs the transaction with the user's wallet and submits it to the network.
 * @param {object} opts
 ...
 * @param {string} opts.redirectLink - full callback URL (with action=signAndSendTransaction)
 */
export function buildSignAndSendTransactionURL({ serializedTx, session, boxKey, redirectLink }) {
  const { publicKey } = getDappKeypair();
  const dappPublicKeyBase58 = bs58.encode(Buffer.from(publicKey));
