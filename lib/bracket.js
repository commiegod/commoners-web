...
/**
 * Tiebreaker: absolute difference between predicted combined score and actual.
 * Entries with no tiebreaker are ranked last among ties (return Infinity).
 * @param {number|null} predicted  Entry's predicted total combined score
 * @param {number|null} actual     Actual championship combined score (winner + loser)
 * @returns {number}
 */
export function tiebreakerDiff(predicted, actual) {
  if (predicted == null || actual == null) return Infinity;
...
export function maxPossibleScore(picks, results) {
  // Build the set of eliminated teams: any team picked for a game that now has
  // a different winner is out and cannot score in any future round.
  const eliminated = new Set();
  for (const [gameId, pick] of Object.entries(picks)) {
    if (pick && results[gameId] && results[gameId] !== pick) {
      eliminated.add(pick);
    }
  }
  ...
  for (const [gameId, pick] of Object.entries(picks)) {
    if (pick && !results[gameId] && !eliminated.has(pick)) {
      score += ROUND_POINTS[getRound(gameId)] ?? 0;
    }
  }
  return score;
