 ...
 *
 * PrimeVils that have been transferred into the MidEvils collection (via the
 * 2-MidEvil burn) are explicitly excluded — they are identified by their
 * `Primevil #N` name prefix. Orcs and any other native MidEvil sub-types are
 * included since they share the `MidEvil #N` naming.
 *
...
/**
 * Returns true if an asset's name marks it as a PrimeVil transferred into the
 * MidEvils collection. These are governance-excluded regardless of trait count.
 */
function isTransferredPrimeVil(asset) {
  const name = asset.content?.metadata?.name ?? "";
  return name.trim().toLowerCase().startsWith("primevil");
}

/**
 * Returns true if an asset is a Commoner — exactly 3 non-"None" traits AND
 * not a transferred PrimeVil.
 */
function isCommoner(asset) {
  if (isTransferredPrimeVil(asset)) return false;
 ...
 * assets, excluding any PrimeVils that have been transferred into the
 * collection (identified by `Primevil #N` name prefix).
 *
