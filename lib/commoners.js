 ...
 * excluding PrimeVils that have been transferred into the MidEvils collection
 * (identified by `Primevil #N` name prefix). Orcs count; transferred
 * PrimeVils do not, regardless of trait count.
 *
...
// Covers the original Commoners snapshot (120 mints, pre-PrimeVil transfers).
 ...
 * - Is not a transferred PrimeVil (name does not start with "Primevil")
 */
function assetIsCommoner(asset) {
  ...
  if (name.trim().toLowerCase().startsWith("primevil")) return false;
  ...
  if (MAINNET_RPC.includes("helius")) {
    try {
      ...
      while (true) {
        ...
        // Dynamic: collection membership + 3 traits + not a transferred PrimeVil.
