 ...
 *
 * Checks whether a connected wallet holds one or more Commoner NFTs.
 * A Commoner is any non-burned MidEvil with exactly 3 non-"None" traits.
 *
 * Uses dynamic on-chain detection — no static mint list required. Saved
 * PrimeVils that join the MidEvils collection are automatically recognised
 * as Commoners without any manual list updates.
 *
 * Returns the number of Commoner NFTs held (voting power = that count).
 */

import commoners from "../data/commoners.json";
import { MIDEVILS_COLLECTION } from "./collection";

// Static fallback Set — used only when Helius DAS is completely unavailable.
// Covers the original Commoners; saved PrimeVils won't count in this path.
const COMMONER_MINTS_FALLBACK = new Set(commoners.nfts.map((n) => n.id));
...
/**
 * Returns true if an asset qualifies as a Commoner:
 * - Is in the MidEvils collection
 * - Has exactly 3 non-"None" traits
 */
function assetIsCommoner(asset) {
  ...
  const activeTraits = attrs.filter(
    (a) => a.value && a.value.toLowerCase() !== "none"
  );
  return activeTraits.length === 3;
}

/**
 * Returns how many Commoner NFTs the given wallet holds (0 = not a holder).
 * Uses the Helius DAS getAssetsByOwner API which handles all token programs
 * and is purpose-built for NFT ownership queries. Falls back to the legacy
 * getTokenAccountsByOwner RPC method if DAS is unavailable.
 *
...
export async function getCommonerCount(walletAddress) {
  ...
  if (MAINNET_RPC.includes("helius")) {
    try {
      ...
      while (true) {
        ...
        // Dynamic: collection membership + exactly 3 traits.
        // Handles original Commoners and saved PrimeVils alike.
        count += items.filter((a) => assetIsCommoner(a)).length;
  ...
  // ── Fallback: legacy getTokenAccountsByOwner ──────────────────────────────
  try {
    const res = await fetch(MAINNET_RPC, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    for (const acct of accounts) {
      ...
      if (
        ...
        COMMONER_MINTS_FALLBACK.has(info.mint)
      ) {
        count++;
      }
    }
 ...
 *   Standard:           51% majority, 20% quorum (24/119 NFTs)
 *   Treasury < 5 SOL:   51% majority, 20% quorum (24/119 NFTs)
 *   Treasury 5-20 SOL:  67% supermajority, 30% quorum (36/119 NFTs)
 *   Treasury > 20 SOL:  75% supermajority + futarchy (Phase 4)
 */
export const TOTAL_NFTS = 119;
