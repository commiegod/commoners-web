...
const CACHE_KEY = "auction_slots_v2";
...
// mint → devnet test NFT metadata (name/image/traits)
const devnetMetaByMint = {};
for (const n of devnetMeta) {
  devnetMetaByMint[n.mint] = n;
}
    ...
    async (conn, skipCache = false) => {
      ...
      try {
        ...
        const result = slotAccounts.map((slot) => {
          ...
          // Only use DAS data if it has an actual name (avoids falling back
          // to das.id which is the raw mint address on devnet)
          if (das && das.content?.metadata?.name) {
            name = das.content.metadata.name;
            image =
          ...
          } else if (devnetMetaByMint[slot.nftMint]) {
            const d = devnetMetaByMint[slot.nftMint];
            name = d.name;
            image = d.image;
            traits = d.traits;
          }
          ...
          return {
            ...
            scheduledDate: slot.scheduledDate, // unix seconds
            nftMint: slot.nftMint,
            name,
            image,
            traits,
            seller: slot.owner,
            reservePrice: slot.reservePrice ? slot.reservePrice.toNumber() : null,
        ...
        for (const [dateStr, entry] of Object.entries(schedule)) {
          if (!onChainMints.has(entry.nftId)) {
            result.push({
              dateStr,
              scheduledDate: new Date(dateStr).getTime() / 1000, // midnight UTC
              nftMint: entry.nftId,
              name: entry.name,
              image: entry.image,
              traits: entry.traits,
        ...
        const fallback = Object.entries(schedule)
          ...
          .map(([dateStr, entry]) => ({
            dateStr,
            scheduledDate: new Date(dateStr).getTime() / 1000,
            nftMint: entry.nftId,
            name: entry.name,
            image: entry.image,
            traits: entry.traits,
