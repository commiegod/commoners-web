...
export function computeMinNextBid(auctionState, bidIncrementBps = 500) {
  const current = auctionState.current_bid;
  if (current.isZero()) {
    return auctionState.reserve_price;
  }
...
export async function fetchSlotAccounts(connection) {
  ...
  return accounts
    ...
    .map(({ pubkey, account }) => {
      ...
      const ts = state.scheduled_date.toNumber();
      return {
        pubkey,
        nftMint: state.nft_mint.toBase58(),
...
export async function fetchActiveAuctions(connection) {
  const coder = new BorshAccountsCoder(idl);
  const accounts = await connection.getProgramAccounts(PROGRAM_ID, {
...
/** PDA for a GovernanceProposal. proposalId is a BigInt or u64-compatible value. */
export function proposalPDA(proposalId) {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("proposal"), bnToLeBuffer(proposalId)],
    PROGRAM_ID
  );
}

/** PDA for a VoteRecord. voter is a PublicKey. */
export function voteRecordPDA(proposalId, voter) {
  return PublicKey.findProgramAddressSync(
    [Buffer.from("vote"), bnToLeBuffer(proposalId), voter.toBuffer()],
    PROGRAM_ID
  );
}

/** Fetch and decode a single GovernanceProposal account. Returns null if not found. */
export async function fetchProposalAccount(connection, proposalId) {
  const [pda] = proposalPDA(proposalId);
  try {
    const info = await connection.getAccountInfo(pda);
    if (!info) return null;
    const coder = new BorshAccountsCoder(idl);
    const state = coder.decode("GovernanceProposal", info.data);
    return { pubkey: pda, state };
  } catch {
    return null;
  }
}

/** Fetch and decode a single VoteRecord account. Returns null if not found (voter hasn't voted). */
export async function fetchVoteRecord(connection, proposalId, voter) {
  const [pda] = voteRecordPDA(proposalId, voter);
  try {
    const info = await connection.getAccountInfo(pda);
    if (!info) return null;
    const coder = new BorshAccountsCoder(idl);
    const state = coder.decode("VoteRecord", info.data);
    return { pubkey: pda, state };
  } catch {
    return null;
  }
